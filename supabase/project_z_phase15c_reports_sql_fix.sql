-- Project Z Phase 15C Reports SQL Fix
-- Fixes:
-- 1. project_z_profiles.full_name does not exist
-- 2. project_z_classes.class_name does not exist
...
as $$
  with teacher_classes as (
    select
      c.id as class_id,
      ('Class ' || left(c.id::text, 8))::text as class_name
...
select
  'Project Z Phase 15C reports SQL fix applied successfully' as status,
  now() as applied_at;
