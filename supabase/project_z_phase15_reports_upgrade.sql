-- Project Z Phase 15 Teacher and Parent Reports Upgrade
-- Safe to run more than once after Phase 14C.
...
as $$
  ...
  profile as (
    select
      a.student_id,
      coalesce(pr.full_name, split_part(pr.email, '@', 1), 'Student') as student_name,
  ...
  mastery as (
    select
      ...
      count(*) filter (where cm.mastery_percent < 45)::integer as weak_skill_count,
      count(*) filter (where cm.mastery_percent >= 45 and cm.mastery_percent < 75)::integer as developing_skill_count,
      count(*) filter (where cm.mastery_percent >= 75)::integer as strong_skill_count
    from allowed a
  ...
  path_counts as (
    select
      a.student_id,
      count(*) filter (where p.path_status = 'mastered_review')::integer as mastered_review_count,
      count(*) filter (where p.path_status = 'locked')::integer as locked_skill_count,
      count(*) filter (where p.path_status = 'ready')::integer as ready_skill_count
    from allowed a
    left join lateral public.project_z_my_skill_path() p on a.student_id = auth.uid()
    group by a.student_id
  ),
  xp as (
    select
...
as $$
  ...
  select
    r.student_id,
    coalesce(p.full_name, split_part(p.email, '@', 1), 'Student') as student_name,
  ...
  order by r.class_name, p.full_name, p.email;
$$;
...
select
  'Project Z Phase 15 teacher and parent reports schema applied successfully' as status,
  now() as applied_at;
